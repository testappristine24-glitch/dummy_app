...
import 'dart:async';
import 'dart:convert';
import 'package:delivoo/Components/loading_shimmer_short.dart';
...
class PlacePickerState extends State<PlacePicker> {
      ...
      body: Column(
        children: <Widget>[
          ...
          if (!this.hasSearchTerm)
            ...
            //     child: Container(height: 30, child: LoadingShimmer()),
            //   )
                : Column(
              children: [
                Container(
  ...
  SelectAddress() {
    ...
    context.read<LocationServiceProvider>().setAddressFromMap(locationResult?.formattedAddress ?? '');

    context.read<LocationServiceProvider>().address = locationResult?.formattedAddress;
    context.read<LocationServiceProvider>().userAddressPinCode = locationResult?.postalCode;
    context.read<LocationServiceProvider>().locationResult = context.read<LocationServiceProvider>()
        .getLatitudeLongitude(locationResult!.latLng!.latitude, locationResult!.latLng!.longitude);
  ...
  void autoCompleteSearch(String place) async {
    try {
      ...
      var endpoint =
          ...
          "input={$place}&sessiontoken=${this.sessionToken}&radius=50000";
  ...
  String getLocationName() {
    ...
    print("location resutlt is printing");
